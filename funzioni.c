	...
	while (fgets(linea, sizeof(linea), fd) != NULL) {
		if (linea[0] == 'j') {
			if (contatore_job != 0) {
				...
				jobs[contatore_job - 1].somma_lunghezza_istruzioni =
						somma_lunghezza_istruzioni;
			}
			sscanf(linea, "%c,%d,%lud", &tipo_linea, &jobs[contatore_job].id,
					&jobs[contatore_job].arrival_time);
			contatore_job++;
			contatore_istruzioni = 0;
			somma_lunghezza_istruzioni = 0;
		} else if (linea[0] == 'i') {
			sscanf(linea, "%c,%d,%d,%d", &tipo_linea,
					(int *) &lista_istruzioni[contatore_istruzioni_totali].type_flag,
					&lista_istruzioni[contatore_istruzioni_totali].lenght,
					&lista_istruzioni[contatore_istruzioni_totali].IO_max);
			if (contatore_istruzioni == 0) {
				jobs[contatore_job - 1].instr_list =
						&lista_istruzioni[contatore_istruzioni_totali];
			} else {
				lista_istruzioni[contatore_istruzioni_totali - 1].successiva =
						&lista_istruzioni[contatore_istruzioni_totali];
			}
			somma_lunghezza_istruzioni +=
					lista_istruzioni[contatore_istruzioni_totali].lenght;
...
void esegui(unsigned long *clock, struct job *puntatore_job) {
	while (1) {
		...
		if (puntatore_job->instr_list->type_flag == 1) { //Istruzione bloccante			
			...
			puntatore_job->arrival_time = random_num(
					puntatore_job->instr_list->IO_max) + *clock;
			break;
	...
	while (1) {
		...
		if (puntatore_job->instr_list->lenght <= quantum) {
			*clock = *clock + puntatore_job->instr_list->lenght;
			...
			puntatore_job->stato = BLOCKED;
			*clock = *clock + quantum;
			puntatore_job->instr_list->lenght =

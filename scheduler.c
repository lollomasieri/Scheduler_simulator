...
void* core_preemptive(void* parameters) {
	...
	struct params_sched_preemptive* params =
			(struct params_sched_preemptive*) parameters;
		...
		for (int i = 0; i < MAX_JOBS; i++) {
			...
			case 0: //NEW
				if (clock >= params->jobs[i].arrival_time) {
					cambiaStato(f_pre, params->core, clock, &params->jobs[i],
							READY, &mutex_file_output_pre);
				}
				break;
			case 1: //READY
				cambiaStato(f_pre, params->core, clock, &params->jobs[i],
						RUNNING, &mutex_file_output_pre);
				flag = 1;
				esegui_quantum(&clock, &(params->jobs[i]), params->quantum);
				cambiaStato(f_pre, params->core, clock, &params->jobs[i],
						params->jobs[i].stato, &mutex_file_output_pre);
				break;
			case 2: //RUNNING
				break;
			case 3: //BLOCKED					
...
void* core_not_preemptive(void* parameters) {
	struct params_sched_not_preemptive* params =
			(struct params_sched_not_preemptive*) parameters;
	...
	while (jobs_rimanenti > 0) {
		...
		for (int i = 0; i < MAX_JOBS; i++) {
			...
			case 0: //NEW
				if (clock >= params->jobs[i].arrival_time) {
					cambiaStato(f_nopre, params->core, clock, &params->jobs[i],
							READY, &mutex_file_output_nopre);
				}
				break;
			case 1: //READY
				cambiaStato(f_nopre, params->core, clock, &params->jobs[i],
						RUNNING, &mutex_file_output_nopre);
				flag = 1;
				esegui(&clock, &(params->jobs[i]));
				cambiaStato(f_nopre, params->core, clock, &params->jobs[i],
						params->jobs[i].stato, &mutex_file_output_nopre);
				break;
			case 2: //RUNNING
				break;
			case 3: //BLOCKED					
